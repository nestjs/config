...
describe('Zod Schema validation', () => {
	...
	describe('Zod schema v3 validation', () => {
		it(`should validate loaded env variables with Zod`, async () => {
			try {
				...
				const message = [{ "code": "invalid_type", "expected": "string", "received": "undefined", "path": ["PORT"], "message": "Required" }, { "code": "invalid_type", "expected": "string", "received": "undefined", "path": ["DATABASE_NAME"], "message": "Required" }]
		...
		it(`should parse loaded env variables with Zod`, async () => {
			const module = await Test.createTestingModule({
				imports: [AppModule.withZodV3SchemaValidation(join(__dirname, '.env.valid'))],
			}).compile();
	...
	describe('Zod schema v4 validation', () => {
		...
		it(`should parse loaded env variables with Zod`, async () => {
			const module = await Test.createTestingModule({
				imports: [AppModule.withZodV4SchemaValidation(join(__dirname, '.env.valid'))],
			}).compile();
	...
	describe('Zod schema v4 mini validation', () => {
		it(`should validate loaded env variables with Zod`, async () => {
			try {
				...
				const message = [{ "expected": "number", "code": "invalid_type", "received": "NaN", "path": ["PORT"], "message": "Invalid input: expected number, received NaN" }, { "expected": "string", "code": "invalid_type", "path": ["DATABASE_NAME"], "message": "Invalid input: expected string, received undefined" }];
				expect(err.message).toEqual(
					'Config validation error: ' + JSON.stringify(message, null, 2),
				);
			}
		...
		it(`should parse loaded env variables with Zod`, async () => {
			const module = await Test.createTestingModule({
				imports: [AppModule.withZodV4MiniSchemaValidation(join(__dirname, '.env.valid'))],
			}).compile();
